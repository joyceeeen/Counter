...
class Counter
{
    ...
    public function allHits($days = null)
    {
        ...
        if ($days) {
            $hits = DB::table($prefix . 'counter_page_visitor')
                ->where('created_at', '>=', Carbon::now()->subDays($days))->count();
        } else {
            $hits = DB::table($prefix . 'counter_page_visitor')->count();
        }

        return number_format($hits);
    }

    public function visitHits($identifier, $id = null, $days = null)
    {
      $page = self::pageId($identifier, $id);

        $prefix = config('database.connections.' . config('database.default') . '.prefix');
        if ($days) {
            $hits = DB::table($prefix . 'counter_page')->where('page',$page)
                ->where('created_at', '>=', Carbon::now()->subDays($days))->value('number_of_hit');
        } else {
            $hits = DB::table($prefix . 'counter_page')->where('page',$page)->value('number_of_hit');
        }
    ...
    private static function hashVisitor()
    {
        ...
        $data = ['visitor_hash'=>hash("SHA256", env('APP_KEY') . $visitor),'ip'=>$ip,'device'=>$device.' ['.$platform.']','browser'=>$browser,'position'=>$position];
    ...
    private static function createVisitorRecordIfNotPresent($visitor)
    {
        ...
        $visitor_record = Visitor::firstOrCreate([
            'visitor' => $visitor['visitor_hash'],
            'country'=>$visitor['position']->countryCode,
            'city'=>$visitor['position']->cityName,
            'latitude'=>$visitor['position']->latitude,
            'longitude'=>$visitor['position']->longitude,
    ...
    {
        return self::$current_page = Page::updateOrCreate(['page' => $page]);
    }
    ...
    private static function createCountIfNotPresent($page)
    {
        ...
        $page_record->increment('number_of_hit');
